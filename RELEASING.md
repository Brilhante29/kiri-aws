...
`CHANGELOG.md`, `version.go`, or the Helm chart version manually.
...
1. **Land a Conventional Commit on `main`.** Pull requests are squash-merged, so
   ...
   format, e.g. `fix(s3): honor the Range header on GetObject`.
2. **release-please keeps a release PR open.** On every push to `main` it
   ...
   (`version.go`, `charts/kiri/Chart.yaml`) and the generated `CHANGELOG.md`
   entry.
...
commit (feat:/fix:) ──► release-please PR ──► merge ──► tag vX.Y.Z
                                     ...
                                     ┌────────────────────┴────────────────────┐
                                     ▼                                         ▼
                        binaries + SBOM + cosign                    Helm chart → ghcr.io
...
| Commit                            | Bump                                     |
| --------------------------------- | ---------------------------------------- |
| `fix:`, `perf:`, `refactor:`      | patch (`0.25.3` → `0.25.4`)              |
| `feat:`                           | minor (`0.25.3` → `0.26.0`)              |
...
- **Multi-arch container image** at
  `ghcr.io/Brilhante29/kiri-aws:{version}` and `:latest`, with the manifest
  signed by cosign.
- **SLSA build provenance** attested to the release archives.
- **Helm chart** at `oci://ghcr.io/Brilhante29/charts/kiri`.
...
```bash
cosign verify ghcr.io/Brilhante29/kiri-aws:<version> \
  --certificate-identity-regexp '^https://github.com/Brilhante29/kiri-aws/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com
```
...
```bash
gh attestation verify kiri_<version>_linux_amd64.tar.gz -R Brilhante29/kiri-aws
```
